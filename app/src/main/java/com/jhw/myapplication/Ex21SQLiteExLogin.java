...
public class Ex21SQLiteExLogin extends AppCompatActivity {

    EditText etId, etPw;

    //SQLITE 데이타베이스 관련변수
    SQLiteDatabase db;
    com.jhw.myapplication.Ex21MySQLiteOpenHelper helper;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main24_sqlite_login_pyke);

        //데이베이스 생성.
        helper = new com.jhw.myapplication.Ex21MySQLiteOpenHelper(
                Ex21SQLiteExLogin.this, // 현재 화면의 context
                "member2.db", // 파일명
                null, // 커서 팩토리
                1); // 버전 번호
        ...
        public void onClick(View v)
        {
            switch(v.getId())
            {
                case R.id.btnLogin:
                    ...
                    if(loginCheck ==true)
                    {
                        Intent login = new Intent(Ex21SQLiteExLogin.this, Ex21SQLiteExMain.class);
                        startActivity(login);
                    }
                    else
                    {
                        Toast.makeText(Ex21SQLiteExLogin.this, "로그인 실패!(아이디/패스워드확인후다시하세요)",  Toast.LENGTH_SHORT).show();
                    }
                    break;
                case R.id.btnJoin:
                    Intent login = new Intent(Ex21SQLiteExLogin.this, Ex21SQLiteExJoin.class);
                    startActivity(login);
                    break;
            }
        }
